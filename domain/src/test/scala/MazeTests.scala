...
class MazeTests extends AnyFreeSpec with OptionValues{
  ...
  "maze" - {
    ...
    "insert players in map should insert all players" in {
      val maze =  Maze(map)
      val players = List(JoiningPlayer(PlayerId(1).value, Username("Bob1").value), JoiningPlayer(PlayerId(2).value, Username("Alice").value))
      val sut =
       maze.insertPlayers(players)
      assert(sut.size == players.size)
      assert(sut.exists(_.id == PlayerId(1).get))
      assert(sut.exists(_.id == PlayerId(2).get))
    ...
    "insert players in map should insert players on available positions" in {
      ...
      assert(sut.size == players.size)
      assert(maze.cells.count(_.cellType == PlayerPosition) == players.size)
      assert(maze.cells.filter(_.cellType == PlayerPosition).map(_.cell).exists(
        cell => (cell.col.value.value == 1 && cell.line.value.value == 1)
