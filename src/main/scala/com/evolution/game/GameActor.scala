...
class GameActor[F[_]: Async] private (
      ...
      gameState match {
        case gameWaiting: GameWaiting =>
          if (gameWaiting.players.size == gameWaiting.nPlayers.value)
            ...
            if (gameWaiting.players.exists(_.id == message.player.id))
              (gameState, GameRepositoryError.PlayerAlreadyInGame.asLeft)
            else {
  ...
  private def processCommand(
      ...
      command: Command,
      instant: Instant
  ): Either[GameFinished, GameRunning] = {
    command match {
      case Command.Tick =>
